...
def serialize_iterable(iterable: Iterable[str], split_symbol: Optional[str] = None) -> str:
  if not isinstance(iterable, Iterable):
    raise ValueError("Parameter 'iterable' is not an Iterable!")
  ...
  if not len(split_symbol) <= 1:
    raise ValueError("Parameter 'split_symbol' needs be at maximum one character!")
  result = ""
  is_first = True
  for item in iterable:
    if not isinstance(item, str):
      raise ValueError(
        f"Iterated item '{item}' could not be serialized because it is not of type 'str'!")
    if item == "":
      raise ValueError(f"Iterated item '{item}' is empty and therefore can not be serialized!")
    if split_symbol != "" and split_symbol in item and item != split_symbol:
      raise ValueError(
        f"Iterated item '{item}' could not be serialized because it contains the split_symbol!")
    if is_first:
      result = item
      is_first = False
    else:
